...
function checkVar(){
    return isset($_POST['login']) && isset($_POST['haslo']) && isset($_POST['email']) ;
}
function checkLength(){
    return strlen($_POST['login'])>0 && strlen($_POST['login'])<=64 && strlen($_POST['haslo'])>0 && strlen($_POST['haslo'])<=64 && strlen($_POST['email'])>0 && strlen($_POST['email'])<=64;
}
        ...
        if (checkEmail()) {
            ...
            if (!$db_connection) {
                ...
                if (mysqli_affected_rows($db_connection) != 0) echo "Login zajety";
                    ...
                    $insert = "INSERT INTO uzytkownik VALUES ('','$login','$haslo','$rola', '$email')";
